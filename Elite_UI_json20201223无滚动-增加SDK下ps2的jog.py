...
class EliteUISDK(QMainWindow):
    def __init__(self):
        ...
        #region 输入框,如Var_M,Output_Y的地址输入框
        self.Var_M = QLineEdit(self)
        self.Var_M.setGeometry(80,650,30,30)
    ...
    def UI(self): 
        ...
        self.btn_methodtest.move(640,920)
        #-------------------------------------------------------------
        #endregion
    ...
    #region 在SDK_on下，调用GetJopstickDataThread实例化后的geejoystickdata读到的数据，并写入显示到窗口
    ...
    def BtnLeftstickUpDown(self,data):
        #print("====",str(self.elt_ctx_self.value))
        ...
        if self.ret_login == 1 and num_data == 0 and self.button_on == 5 and self.SDK_on == 1:
            ...
            if state.value == 3 and num_data == 0:
                ret = Elite_dll.elt_stop(c_void_p(self.elt_ctx_self), byref(error_message))
                self.button_on = 0
        if num_data > 0 and self.ret_login == 1 and self.SDK_on == 1:
            self.button_on = 5
            index = c_long(0)
            index.value = 0
            ret = Elite_dll.elt_jog(c_void_p(self.elt_ctx_self), index, byref(error_message))
    ...
    def SDK_buttonClicked(self):
        ...
        if sender.text() == "Var_M":
            Address_M = c_int(int(self.Var_M.Text()))
            status = c_long(0)
            v_status = 0
